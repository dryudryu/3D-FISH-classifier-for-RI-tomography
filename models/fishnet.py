...
class Bottleneck(nn.Module):
    def __init__(self, inplanes, planes, stride=1, mode='NORM', k=1, dilation=1):
        ...
        self.conv1 = nn.Conv3d(inplanes, btnk_ch, kernel_size=1, bias=False)

        self.bn2 = nn.BatchNorm3d(btnk_ch)
        self.conv2 = nn.Conv3d(btnk_ch, btnk_ch, kernel_size=3, stride=stride, padding=dilation,
                               dilation=dilation, bias=False)

        self.bn3 = nn.BatchNorm3d(btnk_ch)
        self.conv3 = nn.Conv3d(btnk_ch, planes, kernel_size=1, bias=False)
    ...
    def squeeze_idt(self, idt):
        n, c, h, w, z = idt.size()
        return idt.view(n, c // self.k, self.k, h, w, z).sum(2)
...
class Fish(nn.Module):
    ...
    def _make_score(self, in_ch, out_ch=1000, has_pool=False):
        bn = nn.BatchNorm3d(in_ch)
        relu = nn.ReLU(inplace=True)
        conv_trans = nn.Conv3d(in_ch, in_ch // 2, kernel_size=1, bias=False)
        bn_out = nn.BatchNorm3d(in_ch // 2)
        conv = nn.Sequential(bn, relu, conv_trans, bn_out, relu)
        if has_pool:
            fc = nn.Sequential(
                nn.AdaptiveAvgPool3d(1),
                nn.Conv3d(in_ch // 2, out_ch, kernel_size=1, bias=True))
        else:
            fc = nn.Conv3d(in_ch // 2, out_ch, kernel_size=1, bias=True)
        return [conv, fc]

    def _make_se_block(self, in_ch, out_ch):
        bn = nn.BatchNorm3d(in_ch)
        sq_conv = nn.Conv3d(in_ch, out_ch // 16, kernel_size=1)
        ex_conv = nn.Conv3d(out_ch // 16, out_ch, kernel_size=1)
...
class FishNet(nn.Module):
    def __init__(self, block, **kwargs):
        ...
        # resolution: 224x224
        # Load 190212_fishdw_crop140_b32_lr0.0003
        self.conv1 = self._conv_bn_relu(1, inplanes // 2, stride=2)
        self.conv2 = self._conv_bn_relu(inplanes // 2, inplanes // 2)
        self.conv3 = self._conv_bn_relu(inplanes // 2, inplanes)
        self.pool1 = nn.MaxPool3d(3, padding=1, stride=2)
        
        """
        # origin
        self.stem = nn.Sequential(
            self._conv_bn_relu(1, inplanes // 2, stride=2),
            self._conv_bn_relu(inplanes // 2, inplanes // 2),
            self._conv_bn_relu(inplanes // 2, inplanes),
            nn.MaxPool3d(3, padding=1, stride=2)
        )
        """
        """
        self.stem2 = nn.Sequential(
            self._conv_bn_relu(1, inplanes // 4),
            self._conv_bn_relu(inplanes // 4, inplanes // 4),
            self._conv_bn_relu(inplanes // 4, inplanes // 2),
            nn.MaxPool3d(3, padding=1, stride=2),
            self._conv_bn_relu(inplanes // 2, inplanes // 2),
            self._conv_bn_relu(inplanes // 2, inplanes),
            nn.MaxPool3d(2)
...
def fishnetdw(num_classes, **kwargs):
    net_cfg = {
        ...
        'num_res_blks': [2, 8, 16, 8, 4, 4, 4, 4, 4, 8],
        'num_trans_blks': [2, 2, 4, 2, 2, 4],
        'num_cls': num_classes,
        'num_down_sample': 3,
        'num_up_sample': 3,
    }
...
def fishnetdw3(num_classes, **kwargs):
    net_cfg = {
        #  input size:   [224, 56, 28,  14 | 7,   14,  28 | 56,   28,  14]
        # output size:   [56,  28, 14,   7 | 14,  28,  56 | 28,   14,   7]
        #                  |    |    |   |    |    |    |    |     |    |
...
def fishnetdw2(num_classes, **kwargs):
    net_cfg = {
        #  input size:   [224, 56, 28,  14 | 7,   14,  28 | 56,   28,  14]
        # output size:   [56,  28, 14,   7 | 14,  28,  56 | 28,   14,   7]
        #                  |    |    |   |    |    |    |    |     |    |
        'network_planes': [64, 128, 256, 512, 512, 512, 384, 256, 320, 832, 1600],
        'num_res_blks': [4, 8, 16, 8, 4, 8, 8, 8, 8, 16],
        'num_trans_blks': [4, 4, 8, 4, 4, 8],
        'num_cls': num_classes,
        'num_down_sample': 3,
        'num_up_sample': 3,
    }
...
def fishnet150(num_classes, **kwargs):
    """
...
def fishnet99(num_classes, **kwargs):
    """
...
if __name__ == "__main__":
    ...
    summary(net.cuda(), (1, 224, 224, 224))
