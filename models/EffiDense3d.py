...
class _DenseLayer(nn.Module):
    def __init__(self, num_input_features, growth_rate, bn_size, drop_rate, norm, act, efficient=False):
...
class _DenseBlock(nn.Module):
    def __init__(self, num_layers, num_input_features, bn_size, growth_rate, drop_rate, norm, act, efficient=False):
...
class EffiDenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=12, block_config=(16, 16, 16), compression=0.5,
                 ...
                 num_classes=10, norm="bn", act="relu", small_inputs=False, efficient=True):
        ...
        self.features = nn.Sequential(OrderedDict([
            ('conv0', nn.Conv3d(1, num_init_features, kernel_size=3, stride=2, padding=1, bias=False)),
        ]))
        self.features.add_module('norm0', norm(num_init_features))
        self.features.add_module('relu0', act())
        ...
        for i, num_layers in enumerate(block_config):
            ...
            if i != len(block_config) - 1:
                trans = _Transition(num_input_features=self.num_features,
                                    num_output_features=int(self.num_features * compression),
                                    norm=norm, act=act)
                self.features.add_module('transition%d' % (i + 1), trans)
                self.num_features = int(self.num_features * compression)

        # Final batch norm
        self.features.add_module('norm_final', norm(self.num_features))
        self.features.add_module("act_final", act())
    ...
    def forward(self, x):
        ...
        pool = self.avgpool(features).view(features.size(0), -1)
